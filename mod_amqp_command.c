	...
	while (profile->running) {
		...
		while (profile->running && profile->conn_active) {
			...
			if (envelope.message.properties._flags & AMQP_BASIC_CONTENT_TYPE_FLAG) {
				...
				if (strncasecmp("text/json", envelope.message.properties.content_type.bytes, strlen("text/json")) == 0) {
				  	commandFormat = COMMAND_FORMAT_PLAINJSON;
				}else if (strncasecmp("text/plain", envelope.message.properties.content_type.bytes, strlen("text/plain")) == 0) {
					commandFormat = COMMAND_FORMAT_PLAINTEXT;
				} else {
			...
			if (envelope.message.properties.headers.num_entries) {
				...
				for ( x = 0; x < envelope.message.properties.headers.num_entries; x++) {
					...
					if ( !strncmp(header_key, "x-fs-api-resp-exchange", 22)) {
						...
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "MSG body [%s]\n", (char*)envelope.message.body.bytes);
					}
				}
			}

			if (commandFormat == COMMAND_FORMAT_PLAINJSON) {
				switch_event_t *event = NULL;
				char * uuid = NULL;
				char json_str[(int) envelope.message.body.len + 1];
				snprintf(json_str, sizeof(json_str), "%.*s", (int) envelope.message.body.len, (char *) envelope.message.body.bytes);
				switch_event_create_json(&event, (char *) json_str);//json from here...
                if (event){
                    const char *subclass_name = switch_event_get_header(event, "Event-Subclass");
                    char *cmd = switch_event_get_header(event, "call-command");
                    if (subclass_name) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Event-Subclass:%s\n", subclass_name);
                        switch_event_set_subclass_name(event, subclass_name);
                    }else if (cmd){
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "call-command:%s\n", cmd);
                    }
                    if ((uuid = switch_event_get_header(event, "unique_id"))) {
					    switch_core_session_t *dsession;
					    if ((dsession = switch_core_session_locate(uuid))) {
                            if (cmd){
						        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "cmd to private event.subclass:%s\n",subclass_name);
                                switch_core_session_queue_private_event(dsession, &event, SWITCH_FALSE);
                            }else{
						        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "cmd to private not private event subclass:%s\n",subclass_name);
					    ...
					    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "cmd event prep for delivery queue............\n");
					    switch_event_prep_for_delivery(event);
					    switch_event_fire(&event);
                    }
                    switch_event_destroy(&event);
			...
			else if (commandFormat == COMMAND_FORMAT_PLAINTEXT) {
