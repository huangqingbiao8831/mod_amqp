...
switch_status_t mod_amqp_cdr_send(mod_amqp_cdr_profile_t *profile, mod_amqp_message_t *msg)
{
    ...
    if (status < 0) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "Profile[%s] failed to send event on connection[%s]: %s,exchange:%s,routing_key:%s\n",
                          profile->name, profile->conn_active->name, errstr,profile->exchange,msg->routing_key);
