...
int mod_amqp_log_if_amqp_error(amqp_rpc_reply_t x, char const *context)
{
	...
	case AMQP_RESPONSE_LIBRARY_EXCEPTION:
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "response library exception :%s: %s\n", context, amqp_error_string2(x.library_error));
		break;
...
switch_status_t mod_amqp_do_config(switch_bool_t reload)
{
	...
	if (reload) {
		...
		mod_amqp_logging_profile_t *cdr; //add cdr part...
